...
class EmployeeVideosInteractor(private val applicasterClient: ApplicasterClient) {

    suspend fun loadApplicasterEmployeeEntries(): MutableLiveData<EmployeeEntriesState> {
        val liveData =
            MutableLiveData<EmployeeEntriesState>().apply { EmployeeEntriesState.Loading }
        withContext(Dispatchers.IO) {
            applicasterClient.fetchEmployeesVideos { response ->
                liveData.postValue(
                    when (response) {
